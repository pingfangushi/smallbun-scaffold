...
public class SysRoleServiceImpl extends BaseServiceImpl<SysRoleMapper, SysRoleEntity> implements SysRoleService {
	...
	public AjaxResult unique(SysRoleEntity role) {
		...
		if (!StringUtils.isEmpty(role.getId())) {
			queryWrapper = new QueryWrapper<>(role).lambda().eq(false, SysRoleEntity::getRoleName, role.getRoleName())
					.eq(false, SysRoleEntity::getId, role.getId());
		} else {
			queryWrapper = new QueryWrapper<>(role).lambda().eq(false, SysRoleEntity::getRoleName, role.getRoleName());
		}
		return !StringUtils.isEmpty(role.getId()) ?
