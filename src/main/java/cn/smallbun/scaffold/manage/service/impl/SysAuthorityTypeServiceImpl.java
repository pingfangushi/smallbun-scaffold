...
public class SysAuthorityTypeServiceImpl extends
                                         BaseServiceImpl<SysAuthorityTypeMapper, SysAuthorityTypeEntity>
    ...
    public SysAuthorityTypeServiceImpl(RedisClient redisClient,
                                       ISysAuthorizeItemService itemService) {
    ...
    public SysAuthorityTypeEntity getById(Serializable id) {
        List<SysAuthorityTypeEntity> list = this.list().stream()
            .filter(i -> i.getId().equals(id.toString())).collect(Collectors.toList());
    ...
    public boolean updateById(SysAuthorityTypeEntity entity) {
        ...
        if (update) {
            List<SysAuthorityTypeEntity> list = this.list();
            //获取数据
            List<SysAuthorityTypeEntity> values = list.stream()
    ...
    public boolean removeByIds(Collection<? extends Serializable> idList) {
        ...
        if (remove) {
            ...
            idList.forEach(j -> {
                values.clear();
                values.addAll(list.stream().filter(i -> !i.getId().equals(j.toString()))
