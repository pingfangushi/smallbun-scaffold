...
public class SysDictTypeServiceImpl extends BaseServiceImpl<SysDictTypeMapper, SysDictTypeEntity>
    ...
    public boolean save(SysDictTypeEntity entity) {
        ...
        if (save) {
            List<SysDictTypeEntity> list = ApplicationContextHelp.getBean(ISysDictTypeService.class)
                .list();
            list.add(entity);
    ...
    public boolean updateById(SysDictTypeEntity entity) {
        ...
        if (update) {
            ...
            //移除此数据
            List<SysDictTypeEntity> types = list.stream()
                .filter(i -> !i.getId().equals(entity.getId())).collect(Collectors.toList());
            //从数据库查询出来放入缓存
    ...
    public List<Dict> dict() {
        //查询数据
        List<SysDictTypeEntity> typeList = ApplicationContextHelp.getBean(ISysDictTypeService.class)
            .list();
        List<SysDictItemEntity> valueList = ApplicationContextHelp
            .getBean(ISysDictItemService.class).list();
        //处理数据，只选择启用的
        typeList = typeList.stream().filter(i -> DictStatus.ENABLE.equals(i.getStatus()))
            .collect(Collectors.toList());
        ...
        for (SysDictTypeEntity i : typeList) {
            ...
            for (SysDictItemEntity j : valueList) {
                if (i.getId().equals(j.getType())) {
                    // 设置字典项，颜色，标签，值
                    Item item = new Item().setColor(j.getColor()).setLabel(j.getLabel())
                        .setValue(j.getValue());
                    items.add(item);
                    // 设置默认值
    ...
    public boolean removeByIds(Collection<? extends Serializable> idList) {
        ...
        if (remove) {
            ...
            idList.forEach(j -> {
                types.clear();
                types.addAll(list.stream().filter(i -> !i.getId().equals(j.toString()))
                    .collect(Collectors.toList()));
            });
            redis.set(CACHE_NAME, types);
            //删除字典值数据
