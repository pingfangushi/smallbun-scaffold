...
public class SysDictItemServiceImpl extends BaseServiceImpl<SysDictItemMapper, SysDictItemEntity>
    ...
    public boolean updateById(SysDictItemEntity entity) {
        ...
        if (update) {
            List<SysDictItemEntity> list = this.list();
            //获取数据
            List<SysDictItemEntity> values = list.stream()
    ...
    public boolean isExistDefault(Serializable type) {
        List<SysDictItemEntity> list = list().stream().filter(
            i -> i.getType().equals(type.toString()) && i.getIsDefault().equals(DictDefault.YES))
            .collect(Collectors.toList());
    ...
    public boolean removeByTypes(Collection<? extends Serializable> idList) {
        ...
        idList.forEach(i -> {
            //循环根据字典类型删除字典值数据
            flag.set(remove(new LambdaQueryWrapper<SysDictItemEntity>()
                .eq(SysDictItemEntity::getType, i.toString())));
            //删除成功,对缓存进行操作
            if (flag.get()) {
                List<SysDictItemEntity> list = this.list();
                idList.forEach(j -> redis.set(CACHE_NAME,
                    list.stream().filter(ii -> !ii.getType().equals(j.toString()))
                        .collect(Collectors.toList())));
            }
        });
        return flag.get();
