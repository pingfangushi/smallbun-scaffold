 ...
 */
@Service
public class SysRoleServiceImpl extends BaseServiceImpl<SysRoleMapper, SysRoleEntity>
     ...
     */
    private List<RoleAuthVO.RoleAuthItem> getItems(List<SysAuthorizeItemEntity> routes,
                                                   List<RoleAuthBO> bos, AuthorizeType type) {
        //获取此类型的权限原始集合
        routes = routes.stream().filter(i -> i.getType().equals(type)).collect(Collectors.toList());
        //获取拥有此类型的权限集合
        bos = bos.stream().filter(i -> type.getCode().equals(i.getType()))
            .collect(Collectors.toList());
        List<RoleAuthVO.RoleAuthItem> items = Lists.newArrayList();
        //封装所有权限
        for (SysAuthorizeItemEntity route : routes) {
            items.add(getRoleAuthItem(route.getId(), route.getName(), route.getPermission(),
                route.getStatus()));
        }
        //处理拥有权限
     ...
     * @param status 状态
     */
    private RoleAuthVO.RoleAuthItem getRoleAuthItem(String id, String name, String permission,
