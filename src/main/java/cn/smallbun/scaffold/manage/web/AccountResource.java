...
public class AccountResource extends BaseResource {
    ...
    public ResponseEntity<ApiRestResult<LoginResultDTO>> login(@RequestBody LoginDTO login) {
        ...
        if (!validateCaptcha(login.getKey(), login.getCaptcha())) {
            return ResponseEntity.ok(new ApiRestResult<LoginResultDTO>().status(EX000102.getCode())
                .message(EX000102.getMessage()).build());
        }
        // 密码
        try {
            if (!redisClient.hasKey(SECRET_CACHE_NAME + COLON + login.getKey())) {
                logger.error("根据KEY:{}未在缓存中查询到秘钥", login.getKey());
                return ResponseEntity.ok(new ApiRestResult<LoginResultDTO>()
            ...
            String password = RsaUtil.decrypt(RsaUtil.loadPrivateKey(key),
                RsaUtil.strToBase64(login.getPassword()));
        ...
        return new ResponseEntity<>(new ApiRestResult<LoginResultDTO>().result(result).build(),
    ...
    public ApiRestResult<IAccountService.PublicSecretResult> getPublicSecret() {
        return new ApiRestResult<IAccountService.PublicSecretResult>()
            .result(iAccountService.getPublicSecret());
    }
    ...
    public ApiRestResult<IAccountService.ImageCaptchaResult> imageCaptcha(@NotBlank(message = "请求KEY不能为空") String key) throws IOException {
        return new ApiRestResult<IAccountService.ImageCaptchaResult>()
            .result(iAccountService.imageCaptcha(key));
    }
