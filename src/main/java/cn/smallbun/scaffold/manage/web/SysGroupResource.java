...
public class SysGroupResource extends BaseResource {
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增组织", action = ADD, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改组织", action = UPDATE, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除组织", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return new ApiRestResult<>()
            .result(
                sysOrgService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "查询组织列表", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "查询组织树结构", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据ID查询组织", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<GroupVO> getById(@PathVariable String id) {
        return new ApiRestResult<GroupVO>()
            .result(fillingVo(mapping(sysOrgService.getById(id), new GroupVO()))).build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一组织验证", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return 结果
     */
    @Log(feature = "根据ID更新组织类型状态", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:group:update')")
