...
import cn.smallbun.scaffold.manage.pojo.config.ConfigVO;
 ...
 */
@Log(module = SysConfigResource.API)
@Validated
@RestController
...
public class SysConfigResource extends BaseResource {
     ...
     */
    @Log(feature = "新增参数配置", action = ADD, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "修改参数配置", action = UPDATE, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据参数配置ID删除信息", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return new ApiRestResult<>().result(
            sysConfigService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询参数配置信息", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Page<ConfigVO>> getPage(PageModel pageModel, ConfigVO config) {
        // 转换为VO
        Page<ConfigVO> page = pageMapping(sysConfigService.page(pageModel, Wrappers.query(config)),
            ConfigVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "通过参数配置ID查询信息", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:config:fetch')")
