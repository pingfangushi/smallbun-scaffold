...
import cn.smallbun.scaffold.manage.pojo.dict.DictTypeVO;
import cn.smallbun.scaffold.manage.pojo.dict.DictValueVO;
...
public class SysDictResource extends BaseResource {
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增字典类型", action = ADD, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改字典类型", action = UPDATE, platform = Platform.MANAGE)
     ...
     * @return 结果
     */
    @Log(feature = "根据ID更新字典类型状态", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:update')")
    ...
    public ApiRestResult updateStatusById(@PathVariable String id,
                                          @PathVariable DictStatus status) {
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除字典类型", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeTypeByIds(@PathVariable String ids) {
        return new ApiRestResult<>()
            .result(
                typeService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询字典类型列表", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Page<DictTypeVO>> getPage(PageModel pageModel, SysDictTypeEntity type) {
        // 转换为VO
        Page<DictTypeVO> page = pageMapping(typeService.page(pageModel, Wrappers.query(type)),
            DictTypeVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "通过ID查询字典类型", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:fetch')")
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一字典类型验证", action = FETCH, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增字典值", action = ADD, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:add')")
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改字典值", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:update')")
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除字典值", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeValueByIds(@PathVariable String ids) {
        return new ApiRestResult<>().result(
            valueService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))));
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询字典值列表", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:fetch')")
    ...
    public ApiRestResult<Page<DictValueVO>> getPage(PageModel model, SysDictItemEntity value) {
        Page<DictValueVO> page = pageMapping(valueService.page(model, Wrappers.query(value)),
            DictValueVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据ID查询字典值", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:fetch')")
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一字典值验证", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:unique')")
     ...
     * @return 结果
     */
    @Log(feature = "根据ID更新字典值是否为默认", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:dict:update')")
    ...
    public ApiRestResult updateDefaultById(@PathVariable String id,
                                           @PathVariable DictDefault isDefault) {
        // 只有设置默认值的时候进行验证
        if (DictDefault.YES.equals(isDefault)) {
            if (valueService.isExistDefault(valueService.getById(id).getType())) {
                return new ApiRestResult<>().status("S2000").message("该类型下已设置默认项,请取消后在进行设置!")
                    .build();
            }
        }
        return new ApiRestResult<>().result(valueService.updateIsDefaultById(id, isDefault))
            .build();
    }
    ...
    public ApiRestResult<List<ISysDictTypeService.Dict>> query() {
        return new ApiRestResult<List<ISysDictTypeService.Dict>>().result(typeService.dict())
            .build();
    }
