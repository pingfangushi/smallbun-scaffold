...
import cn.smallbun.scaffold.manage.pojo.authority.AuthorityVO;
import cn.smallbun.scaffold.manage.pojo.authority.AuthorizeItemVO;
...
public class SysAuthorityResource extends BaseResource {
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增系统权限", action = ADD, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Boolean> add(@Validated(value = { AddGroup.class }) @RequestBody SysAuthorityTypeEntity authority) {
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改系统权限", action = UPDATE, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Boolean> updateById(@Validated(value = { UpdateGroup.class }) @RequestBody SysAuthorityTypeEntity authority) {
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除权限", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return ApiRestResult
            .ok(typeService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询权限列表", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Page<AuthorityVO>> getPage(PageModel pageModel,
                                                    SysAuthorityTypeEntity authority) {
        // 转换为VO
        Page<AuthorityVO> page = pageMapping(typeService.page(pageModel, Wrappers.query(authority)),
            AuthorityVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "通过ID查询权限", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:authority:fetch')")
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一权限验证", action = FETCH, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增权限项", action = ADD, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:authority:add')")
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改权限项", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:authority:update')")
    ...
    public ApiRestResult updateItemById(@Validated(value = { UpdateGroup.class }) @RequestBody SysAuthorizeItemEntity authority) {
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除权限项", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeItemByIds(@PathVariable String ids) {
        return ApiRestResult
            .ok(itemService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "查询权限项列表", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:authority:fetch')")
    ...
    public ApiRestResult<List<AuthorizeItemVO>> getItemList(SysAuthorizeItemEntity authority) {
        // 转换为VO
        List<AuthorizeItemVO> list = listMapping(itemService.list(Wrappers.query(authority)),
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据ID查询权限项", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:authority:fetch')")
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一权限项验证", action = FETCH, platform = Platform.MANAGE)
    ...
    public SysAuthorityResource(ISysAuthorizeItemService itemService,
