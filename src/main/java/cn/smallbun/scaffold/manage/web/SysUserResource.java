...
import cn.smallbun.scaffold.manage.pojo.user.UserVO;
...
public class SysUserResource extends BaseResource {
     ...
     */
    @DemoEnvironment
    @Log(feature = "新增用户", action = ADD, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "修改用户", action = UPDATE, platform = Platform.MANAGE)
     ...
     */
    @DemoEnvironment
    @Log(feature = "根据ID删除用户", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return new ApiRestResult<>().result(
            iSysUserService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询用户列表", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据ID查询用户", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "唯一用户验证", action = FETCH, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "设置用户密码", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Boolean> updatePassWord(String id, String passWord) {
        return new ApiRestResult<Boolean>().result(iSysUserService.updatePassWord(id, passWord))
            .build();
    }
     ...
     * @return 结果
     */
    @Log(feature = "根据ID更新用户类型状态", action = UPDATE, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:user:update')")
