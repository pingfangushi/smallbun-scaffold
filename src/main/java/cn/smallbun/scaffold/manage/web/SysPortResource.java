...
import cn.smallbun.scaffold.manage.pojo.port.PortVO;
...
@Api(tags = API)
@Log(module = API)
@RequestMapping(PathConstant.MANAGE_API_PATH + "/port")
public class SysPortResource extends BaseResource {
     ...
     */
    @Log(feature = "新增岗位", action = ADD, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "修改岗位", action = UPDATE, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据岗位ID删除信息", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return new ApiRestResult<>().result(
            iSysPortService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
    ...
    public ApiRestResult<Page<PortVO>> getPage(PageModel pageModel, PortVO port) {
        // 转换为VO
        Page<PortVO> page = pageMapping(iSysPortService.page(pageModel, Wrappers.query(port)),
            PortVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "通过岗位ID查询信息", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:port:fetch')")
