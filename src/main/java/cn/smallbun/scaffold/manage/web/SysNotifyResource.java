...
import cn.smallbun.scaffold.manage.pojo.notify.NotifyVO;
...
public class SysNotifyResource extends BaseResource {
     ...
     */
    @Log(feature = "新增通知公告", action = ADD, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "修改通知公告", action = UPDATE, platform = Platform.MANAGE)
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "根据通知公告ID删除信息", action = REMOVE, platform = Platform.MANAGE)
    ...
    public ApiRestResult removeByIds(@PathVariable String ids) {
        return new ApiRestResult<>().result(
            iSysNotifyService.removeByIds(Lists.newArrayList(ids.split(StringUtil.SPLIT_DEFAULT))))
            .build();
    }
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "分页查询岗位信息", action = FETCH, platform = Platform.MANAGE)
    ...
    public ApiRestResult<Page<NotifyVO>> getPage(PageModel pageModel, NotifyVO notify) {
        // 转换为VO
        Page<NotifyVO> page = pageMapping(iSysNotifyService.page(pageModel, Wrappers.query(notify)),
            NotifyVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "查询用户信息", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:notify:fetch')")
    ...
    public ApiRestResult<List<NotifyVO>> getList(@Valid NotifyVO notify) {
        // 转换为VO
        List<NotifyVO> list = listMapping(iSysNotifyService.list(Wrappers.query(notify)),
            NotifyVO.class);
     ...
     * @return {@link ApiRestResult} 通用返回对象
     */
    @Log(feature = "通过通知公告ID查询信息", action = FETCH, platform = Platform.MANAGE)
    @PreAuthorize("hasAuthority('manage:interface:notify:fetch')")
