 ...
 */
@Logging(module = SysNotifyResource.API)
@Validated
@RestController
...
public class SysNotifyResource extends BaseResource {
	 ...
	 */
	@Logging(feature = "新增通知公告", action = ADD, platform = Platform.MANAGE)
	 ...
	 * @return {@link ApiRestResult} 通用返回对象
	 */
	@Logging(feature = "修改通知公告", action = UPDATE, platform = Platform.MANAGE)
	 ...
	 * @return {@link ApiRestResult} 通用返回对象
	 */
	@Logging(feature = "根据通知公告ID删除信息", action = REMOVE, platform = Platform.MANAGE)
	 ...
	 * @return {@link ApiRestResult} 通用返回对象
	 */
	@Logging(feature = "查询用户信息", action = FETCH, platform = Platform.MANAGE)
	@PreAuthorize("hasAuthority('manage:interface:notify:fetch')")
	 ...
	 * @return {@link ApiRestResult} 通用返回对象
	 */
	@Logging(feature = "通过通知公告ID查询信息", action = FETCH, platform = Platform.MANAGE)
	@PreAuthorize("hasAuthority('manage:interface:notify:fetch')")
