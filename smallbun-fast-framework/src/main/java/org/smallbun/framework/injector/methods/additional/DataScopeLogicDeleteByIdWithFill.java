 ...
 * the License.
 */
package org.smallbun.framework.injector.methods.additional;
 ...
 * @since 2018-11-09
 */
public class DataScopeLogicDeleteByIdWithFill extends AbstractDataScopeLogicMethod {
	...
	public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
		...
		if (tableInfo.isLogicDelete()) {
			...
			if (CollectionUtils.isNotEmpty(fieldInfos)) {
				String sqlSet =
						"SET " + fieldInfos.stream().map(i -> i.getSqlSet(EMPTY)).collect(joining(EMPTY)) + tableInfo
								.getLogicDeleteSql(false, true);
